...
mod tests {
    ...
    fn test_list_all_devices() {
        ...
        unsafe {
            ...
            let ret = cbinding::list_all_devices(&mut device_handles_output, &mut output_len);
            assert!(matches!(ret, cbinding::error_code::ok));
            assert_eq!(output_len, 2);

            cbinding::destroy_device_handles(device_handles_output, output_len);
        }
    ...
    fn test_get_device_by_index() {
        ...
        unsafe {
            ...
            let mut ret = cbinding::get_device_by_index(0_u8, &mut device_handle_output);
            assert!(matches!(ret, cbinding::error_code::ok));

            let mut index_output: u8 = 0;
            ret = cbinding::device::get_device_index(device_handle_output, &mut index_output);
            assert!(matches!(ret, cbinding::error_code::ok));
            assert_eq!(index_output, 0);

            cbinding::destroy_device_handle(device_handle_output);
        }

        unsafe {
            let mut device_handle_output: cbinding::device_handle = std::ptr::null_mut();

            let mut ret = cbinding::get_device_by_index(1_u8, &mut device_handle_output);
            assert!(matches!(ret, cbinding::error_code::ok));
    ...
    fn test_get_device_file_by_name() {
        ...
        unsafe {
            ...
            let ret = cbinding::get_device_file_by_name(device_name, &mut device_file_output);
            assert!(matches!(ret, cbinding::error_code::ok));
            ...
            cbinding::destroy_device_file(device_file_output);
        }
    ...
    fn test_device_apis() {
        ...
        unsafe {
            ...
            let mut ret = cbinding::list_all_devices(&mut device_handles_output, &mut output_len);
            assert!(matches!(ret, cbinding::error_code::ok));
            assert_eq!(output_len, 2);
            ...
            for device_handle_idx in 0..output_len {
                ...
                ret = cbinding::device::get_device_index(*device_handle, &mut device_index_output);
                assert!(matches!(ret, cbinding::error_code::ok));
                ...
                ret = cbinding::device::get_device_error_states(
                    *device_handle,
                ...
                cbinding::device::destroy_error_states(err_stats_output, err_state_len);

                let mut pci_bus_num_output: *mut c_char = ptr::null_mut();
                ret = cbinding::device::get_device_pci_bus_number(
                    *device_handle,
                ...
                cbinding::device::free_string(pci_bus_num_output);

                let mut pci_dev_id_output: *mut c_char = ptr::null_mut();
                ret =
                    cbinding::device::get_device_pci_dev_id(*device_handle, &mut pci_dev_id_output);
                assert!(matches!(ret, cbinding::error_code::ok));
                let pci_dev_id =
                    String::from_utf8_lossy(CStr::from_ptr(pci_dev_id_output).to_bytes())
                        .to_string();
                assert!(pci_dev_id == "234:0" || pci_dev_id == "510:0");
                cbinding::device::free_string(pci_dev_id_output);

                let mut serial_number_output: *mut c_char = ptr::null_mut();
                ret = cbinding::device::get_device_serial_number(
                    *device_handle,
                ...
                cbinding::device::free_string(serial_number_output);

                let mut device_uuid_output: *mut c_char = ptr::null_mut();
                ret = cbinding::device::get_device_uuid(*device_handle, &mut device_uuid_output);
                assert!(matches!(ret, cbinding::error_code::ok));
                let device_uuid =
                ...
                cbinding::device::free_string(device_uuid_output);

                let mut device_firmware_version_output: *mut c_char = ptr::null_mut();
                ret = cbinding::device::get_device_firmware_version(
                    *device_handle,
                ...
                cbinding::device::free_string(device_firmware_version_output);

                let mut device_driver_version_output: *mut c_char = ptr::null_mut();
                ret = cbinding::device::get_device_driver_version(
                    *device_handle,
                ...
                cbinding::device::free_string(device_driver_version_output);

                let mut heartbeat_output: u32 = 0;
                ret = cbinding::device::get_device_heartbeat(*device_handle, &mut heartbeat_output);
                assert!(matches!(ret, cbinding::error_code::ok));
                assert_eq!(heartbeat_output, 14649);

                let mut numa_node_output: u8 = 0;
                ret = cbinding::device::get_device_numa_node(*device_handle, &mut numa_node_output);
                assert!(matches!(
                    ret,
                ...
                ret = cbinding::device::get_device_core_num(*device_handle, &mut core_num_output);
                assert!(matches!(ret, cbinding::error_code::ok));
                assert_eq!(core_num_output, 2);

                let mut core_ids_output: *mut u8 = ptr::null_mut();
                let mut core_ids_len: u8 = 0;
                ret = cbinding::device::get_device_core_ids(
                    *device_handle,
                    &mut core_ids_output,
                    &mut core_ids_len,
                );
                ...
                cbinding::device::destroy_device_core_ids(core_ids_output, core_ids_len);

                let mut device_files_output: *mut cbinding::device::DeviceFile = ptr::null_mut();
                let mut device_files_len: u8 = 0;
                cbinding::device::get_device_files(
                    *device_handle,
                ...
                ret = cbinding::device::get_device_all_core_status(
                    *device_handle,
                ...
                cbinding::device::destroy_core_status_pair(
